...
====
'''
Barra lateral: Enlace permanente de la barra lateral: Contenedores/rh-os-n_use_case_openshift_virtualization_trident_install.html palabras clave: OpenShift, OCP, Trident, Trident Protect, NetApp ONTAP, Red Hat OpenShift, virtualización OpenShift, Red Hat OpenShift Resumen de la virtualización: Virtualización de Red Hat OpenShift con NetApp ONTAP ---
...
[%collapsible%open]
====
Ahora puede establecer la clase de almacenamiento Trident necesaria y la clase de snapshot para volúmenes como la predeterminada en el clúster de OpenShift.

Como se ha mencionado anteriormente, es necesario definir la clase de almacenamiento predeterminada y la clase de instantánea de volumen para permitir que OpenShift Virtualization ponga el origen de imagen principal a disposición para crear máquinas virtuales a partir de plantillas predeterminadas.
