...
summary: En esta sección se proporcionan los pasos detallados necesarios para configurar GPFS y mover datos a NFS mediante NetApp XCP. 
---
...
link:bda-ai-business-benefits.html["Anterior: Ventajas empresariales."]
...
....
. Especifique el binario de shell remoto que debe utilizar GPFS; utilice `-r argument`.
+
....
...
[root@mastr-51 installer]#
....
. Especifique el binario de la copia remota que debe utilizar GPFS; utilice `-rc argument`.
+
....
...
[root@mastr-51 installer]#
....
. Especifique el rango de puertos que se establecerá en todos los nodos GPFS; utilice `-e argument`.
+
....
...
[root@mastr-51 cluster-test]#
....
. Compruebe el GPFS de lectura y escritura, mediante la ejecución del `dd` comando.
+
....
...
link:bda-ai-mapr-fs-to-ontap-nfs.html["Siguiente: MAPR-FS para ONTAP NFS."]
