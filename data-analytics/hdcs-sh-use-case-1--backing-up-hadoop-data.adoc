...
La solución A agrega este módulo de análisis in situ al clúster Hadoop de backup, que permite realizar backups secundarios en sistemas de almacenamiento NFS de NetApp y elimina los requisitos de cinta, como se muestra en la siguiente figura.
...
* Ejecutando el `Hadoop distcp` Comando aprovechando MapReduce y varios asignadores, los datos de análisis están protegidos del clúster de Hadoop de backup a NFS mediante el módulo de análisis in situ.
+
...
La solución B agrega el módulo de análisis in situ al clúster de producción de Hadoop, lo que elimina la necesidad de tener el clúster de Hadoop de backup, como se muestra en la siguiente figura.
...
+
Hadoop de forma nativa `hadoop distcp` El comando protege los datos de Hadoop del clúster de producción HDFS a NFS mediante el módulo de análisis in situ.
