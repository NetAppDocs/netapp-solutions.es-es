...
[%collapsible]
====
Con los requisitos previos aprovisionados, inicie sesión en la instancia de EC2 como usuario raíz para configurar el kernel de Linux para la instalación de Oracle.
...
[%collapsible]
====
Aprovisione tres volúmenes desde la consola FSX para alojar los archivos binarios, de datos y registros de la base de datos Oracle.
...
. Inicie sesión en la instancia de EC2 por SSH como usuario ec2. A continuación, cambie a la clave SSH y a la dirección IP de la instancia de EC2.
+
....
...
....
. Cambie la propiedad de los dispositivos a. `oracle:oinstall`.
+
....
...
....
. Como usuario de Oracle, inicie `gridSetup.sh` para la instalación de la infraestructura grid.
+
....
...
. Como usuario root, ejecute los scripts siguientes:
+
....
...
....
. Si ha recibido el mensaje "[INS-43080] algunos de los asistentes de configuración han fallado, han sido cancelados o omitidos", ejecute el siguiente comando como usuario de oracle para comprobar si se creó el recurso de grupo de discos DE DATOS.
+
....
bin/crsctl stat res -t
....
. Si no se creó el grupo de discos DE DATOS, vuelva a cargar el multipathd como usuario root.
+
....
systemctl restart multipathd
....
. Como usuario oracle, cree un grupo de discos de DATOS si no se creó durante la instalación de grid.
+
....
...
....
. Ejecute la instalación silenciosa de la base de datos de sólo software.
+
....
 ./runInstaller -applyRU /tmp/archive/34765931/ -silent -ignorePrereqFailure -responseFile /tmp/archive/dbinstall.rsp
....
. Ejecute el `root.sh` secuencia de comandos después de la instalación sólo de software.
+
....
/u01/app/oracle/product/19.0.0/db1/root.sh
....
. Cree el `dbca.rsp` archivo con las siguientes entradas:
+
....
...
totalMemory=8192
....
. Creación de bases de datos Lauch con dbca.
+
....
...
....
. Validar Oracle Restart ha Services tras la creación de la base de datos.
+
....
