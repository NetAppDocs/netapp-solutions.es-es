...
Trident permite el consumo y la gestión de recursos de almacenamiento en todas las plataformas de almacenamiento más conocidas de NetApp, ya sea en el cloud público o en las instalaciones, incluidos ONTAP (AFF, FAS, Select, Cloud, Amazon FSx ONTAP), el software Element (NetApp HCI, SolidFire), el servicio Azure NetApp Files y Google Cloud NetApp Volumes en Google Cloud. Trident es una interfaz de almacenamiento de contenedores (CSI) que ordena el almacenamiento dinámico conforme a la normativa que se integra de forma nativa con Kubernetes.
