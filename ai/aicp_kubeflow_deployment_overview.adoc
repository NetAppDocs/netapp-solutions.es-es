...
summary: En esta página se describen las tareas que debe completar para poner en marcha Kubeflow en su clúster de Kubernetes. 
---
...
. Ya tiene un clúster de Kubernetes en funcionamiento y ejecuta una versión de Kubernetes que admite ubeflow. Para obtener una lista de las versiones compatibles, consulte https://www.kubeflow.org/docs/started/getting-started/["Documentación oficial de Kubeflow"^].
. Ya ha instalado y configurado NetApp Trident en su clúster de Kubernetes, como se indica en link:aicp_netapp_trident_deployment_and_configuration_overview.html["Implementación y configuración de Trident"].
...
NOTE: Como alternativa, puede implementar Kubeflow manualmente siguiendo la https://www.kubeflow.org/docs/started/getting-started/["instrucciones de instalación"^] En la documentación oficial de Kubeflow

. Implemente Kubeflow en su clúster siguiendo el https://github.com/NVIDIA/deepops/blob/master/docs/k8s-cluster/kubeflow.md["Instrucciones de despliegue de Kubeflow"^] En el sitio de NVIDIA DeepOps GitHub.
...
....
. Confirmar que todos los POD implementados en el espacio de nombres Kubeflow muestran un `STATUS` de `Running` y confirmar que ningún componente puesto en marcha en el espacio de nombres se encuentra en un estado de error. El inicio de todos los pods puede tardar varios minutos.
+
....
...
+
El nombre de usuario predeterminado es `admin@kubeflow.org`, y la contraseña predeterminada es `12341234`. Para crear usuarios adicionales, siga las instrucciones de https://www.kubeflow.org/docs/components/multi-tenancy/["Documentación oficial de Kubeflow"^].



image:aicp_image8.png["Error: Falta la imagen gráfica"]

link:aicp_example_kubeflow_operations_and_tasks_overview.html["Siguiente: Ejemplo de operaciones y tareas de Kubeflow."]
