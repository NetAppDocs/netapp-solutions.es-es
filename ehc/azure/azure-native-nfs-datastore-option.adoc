...
NOTE: Esta función se encuentra actualmente en vista previa pública.
...
* Utilice el nivel Premium o Ultra para un rendimiento óptimo.
...
Para obtener más información acerca del rendimiento de Azure NetApp Files Volume por tamaño o cuota, consulte https://docs.microsoft.com/en-us/azure/azure-netapp-files/azure-netapp-files-performance-considerations["Consideraciones de rendimiento para Azure NetApp Files"^].
...
| Puntos que hay que recordar 
 ...
 a| 
* Use el nivel Premium o Ultra para volúmenes de almacenes de datos con un rendimiento óptimo.
* Para los requisitos de uso compartido de archivos para las máquinas virtuales invitados, use volúmenes de niveles Standard o Premium.
...
Si bien la cantidad recomendada de máquinas virtuales por almacén de datos NFS es subjetiva, hay muchos factores que determinan la cantidad óptima de máquinas virtuales que pueden colocarse en cada almacén de datos. Aunque la mayoría de los administradores solo consideran la capacidad, la cantidad de I/o simultáneas que se envían a los VMDK es uno de los factores más importantes para el rendimiento general. El host ESXi tiene muchos mecanismos para garantizar la justicia entre las máquinas virtuales que compiten por los recursos de almacenes de datos. Sin embargo, la forma más sencilla de controlar el rendimiento es regular cuántas máquinas virtuales se colocan en cada almacén de datos. Si los patrones de I/o simultáneos de las máquinas virtuales envían demasiado tráfico al almacén de datos, se llenan las colas de discos y se genera una latencia más alta.



== Ajuste de tamaño de volúmenes y almacenes de datos

Cuando se crea un volumen en Azure NetApp Files con fines de almacén de datos, la práctica recomendada es crear un volumen no mayor al necesario. Aunque el tamaño máximo de volumen puede ser de hasta 100 TB, NetApp recomienda comenzar con una capacidad de almacén de datos pequeña y aumentarla según se requiera. Los almacenes de datos con ajuste de tamaño adecuado evitan que se coloque de forma accidental demasiados equipos virtuales en el almacén de datos y reducen la probabilidad de que se produzca una contención de recursos. Dado que los tamaños de almacenes de datos y VMDK se pueden aumentar fácilmente si un equipo virtual necesita capacidad adicional, no es necesario crear almacenes de datos mayores de lo necesario. Para obtener un rendimiento óptimo, se recomienda aumentar el número de almacenes de datos en lugar de aumentar el tamaño.
...
| Puntos que debe recordar 
 ...
 a| 
* Un buen tamaño para un almacén de datos ANF NFS es de 4 TB a 8 TB.
* Coloque 15-20 equipos virtuales en un único almacén de datos. En función de los requisitos del equipo virtual, esto puede aumentarse a 35-40 equipos virtuales.
* Para obtener el mejor rendimiento y capacidad de gestión, considere sistemas de archivos propiedad de invitado como sistemas de archivos NFS/SMB gestionados por los invitados para aplicaciones de alto I/o como bases de datos.
...
* Durante la vista previa pública, VAAI no se habilita.
