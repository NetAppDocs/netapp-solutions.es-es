...
[%collapsible]
====
El primer paso es configurar Cloud Volumes ONTAP en Azure (https://["Enlace"^]) Y replicar los volúmenes deseados en Cloud Volumes ONTAP con las frecuencias y retentions de instantánea deseadas.
...
Para configurar AVS SDDC (ya sea a petición o en modo piloto), consulte https://["Ponga en marcha y configure el entorno de virtualización en Azure"^]. Como requisito previo, verifique que los equipos virtuales invitados que residen en los hosts AVS pueden consumir datos de Cloud Volumes ONTAP una vez establecida la conectividad.
