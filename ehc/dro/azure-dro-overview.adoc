...
* Compruebe que ha habilitado la replicación entre regiones mediante la creación de pares de replicación. Consulte https://["Crear replicación de volúmenes para Azure NetApp Files"^].
...
* Configure el https://["replicación"^] programe cada volumen de forma apropiada según las necesidades empresariales y la tasa de cambio de datos.
...
https://["Azure NetApp Files"^] es un servicio de almacenamiento de archivos de uso medido, de nivel empresarial y de alto rendimiento. Siga los pasos que se indican a continuación https://["enlace"^] Para aprovisionar y configurar Azure NetApp Files como almacén de datos NFS para optimizar las puestas en marcha de cloud privado de AVS.
...
Siga los pasos que se indican a continuación https://["enlace"^] para configurar la replicación entre regiones mediante la creación de pares de replicación. El nivel de servicio del pool de capacidad de destino puede coincidir con el del pool de capacidad de origen. Sin embargo, para este caso de uso específico, puede seleccionar el nivel de servicio estándar y luego https://["modificar el nivel de servicio"^] En caso de desastre real o simulaciones de recuperación ante desastres.
...
. Introduzca el `cloudadmin@vsphere.local` credenciales de usuario. A esto se puede acceder desde Azure Portal. Siga los pasos mencionados en este https://["enlace"^]. Una vez hecho esto, haga clic en *Continuar*.
+
